...
! Author: psfriso
!
! Created on September 14, 2012, 2:48 PM
!
!----------------------------------------------------------------------
!----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
PRIVATE
PUBLIC pdbsuperpos, rmsd,pythag,svdcmp
  integer, parameter :: dp = kind(1.d0)

!-----------------------------------------------------------------------
CONTAINS
!-----------------------------------------------------------------------
SUBROUTINE eval_evo(natom,nres,r,ica,rtarg,dorig,frac)
use geometryDP
 IMPLICIT NONE
 real, INTENT(OUT) :: frac
 TYPE (pointDP), INTENT(IN), DIMENSION(natom) :: r,rtarg
 REAL :: dorig
 ...
 accum=0.
 DO i=1,nres
  accum=accum + calcDistDP(r(ica(i)),rtarg(ica(i)))
 END DO
 frac=accum/dorig
 END SUBROUTINE eval_evo

!-----------------------------------------------------------------------
SUBROUTINE pdbsuperpos(natom, U , center1, center2,r)
! superpone rota y traslada r (U, center1+center2)
use geometryDP
INTEGER, INTENT(IN):: natom
 real(dp), dimension(3,natom) :: x
real(dp), dimension(3,3), intent(in) :: U
real(dp), dimension(3), intent(in) :: center1, center2
TYPE(pointDP), intent(inout):: r(natom)!, rtarg(natom)
!
INTEGER i
do i =1,natom
 x(1,i) = r(i)%x
...
enddo
    do i=1, natom
      x(1:3,i)=matmul(U, x(1:3,i)-center2)+center1
      r(i)%x=x(1,i)
      r(i)%y=x(2,i)
      r(i)%z=X(3,i)
    end do
...
END SUBROUTINE pdbsuperpos
!-----------------------------------------------------------------------
!-----------------------------------------------------------------------
SUBROUTINE pdbsuperposCA(nres , coord2 , U , center1, center2, rtarg)
use geometryDP
INTEGER, INTENT(IN):: nres
! INTEGER, DIMENSION(nres), INTENT(IN) :: ica
real(dp), dimension(:,:), intent(inout) :: coord2
real(dp), dimension(3,3), intent(in) :: U
real(dp), dimension(3), intent(in) :: center1, center2
TYPE(pointDP), intent(inout):: rtarg(nres)
!
INTEGER i
    do i=1, nres
      coord2(1:3,i)=matmul(U, coord2(1:3,i)-center2)+center1
      rtarg(i)%x=coord2(1,i)
      rtarg(i)%y=coord2(2,i)
      rtarg(i)%z=coord2(3,i)
    end do


END SUBROUTINE pdbsuperposCA
!-----------------------------------------------------------------------


!-----------------------------------------------------------------------
SUBROUTINE pdbsuperpos2(natom, U , center1, center2,r)
use geometryDP
INTEGER, INTENT(IN):: natom
...
TYPE(pointDP), intent(inout):: r(natom)!, rtarg(natom)
! real aux1,aux2,aux3
real , dimension(3) :: aux4=0!,aux5
!
INTEGER i


    do i=1, natom
  !    aux1=r(i)%x !=coord1(1,i)
  !    aux2=r(i)%y !=coord1(2,i)
  !    aux3=r(i)%z !=coord1(3,i)
  !    aux5(1:3,i)=(/r(i)%x,r(i)%y,r(i)%z/)
      aux4(1:3)=matmul(U, (/r(i)%x,r(i)%y,r(i)%z/)-center2 )+center1
      r(i)%x=aux4(1) ; r(i)%y=aux4(2) ; r(i)%z=aux4(3)
    end do


END SUBROUTINE pdbsuperpos2
!-----------------------------------------------------------------------
...
USE geometryDP
!
  TYPE(pointDP), INTENT(IN), DIMENSION(n) :: coord1, coord2
...
! Put coordinates the way is understanded here
 x(1,1:n) = coord1(1:n)%x
 y(1,1:n) = coord2(1:n)%x
 x(2,1:n) = coord1(1:n)%y
 y(2,1:n) = coord2(1:n)%y
 x(3,1:n) = coord1(1:n)%z
 y(3,1:n) = coord2(1:n)%z
