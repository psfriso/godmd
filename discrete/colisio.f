...
!===============================================================================
subroutine colisioNonBond(stepPts, temps, nblist, r, v, xm, ierr, &
    TMIN, natom, isolv)
    use geometry
    use geometryDP
    use intList
  ...
  !      write(*,*) " Check 0"
        do k = 1, nblist(i) % nats
            ...
            if (j .gt. i ) then
    !            write(*,*) "    check1.1.1"
           !     write(*,*) r(i)%x,r(j)%x,v(i)%x,v(j)%x,rij,csang,nblist(i)%idata(k)%stepPt%dmin,i,j,k
                csang = calcSignCosAng(r, v, i, j, natom)
